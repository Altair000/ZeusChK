...
def update_proxies_list(file_stream):
    ...
    try:
        ...
        bot.send_message(message.chat.id, f"Error al leer el archivo: {e}")
    except ValueError as e:
        bot.send_message(message.chat.id, f"Error al procesar los datos del archivo: {e}")
...
def filter_and_update_proxies(proxies_list, progress_message_id):
    ...
    for i, (ip, details) in enumerate(proxies_list.items(), 1):
        ...
        if i % max(1, total_proxies // 100) == 0 or i == total_proxies:
